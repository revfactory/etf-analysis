...
## 부속 조항: 숫자 포매팅 규칙 (2026-04-14 채택)

PDF↔웹 간 "같은 숫자, 다른 표기" 드리프트 차단. 공용 `fmt.*` 함수 명세.

| 항목 | 함수 | 규칙 | 예시 |
|---|---|---|---|
| AUM | `fmt.aum(n)` | 억원 고정, 천단위 콤마, 반올림 정수 | "6,518 억원" |
| 비중 | `fmt.pct(n, {decimals:2})` | 소수점 2자리 + "%" | "10.49%" |
| 비중 변화 | `fmt.pp(n)` | 부호 강제(+/-/0) + 소수점 2자리 + "%p" | "+2.99%p" |
| bp 보조 | `fmt.bp(n)` | %p × 100 반올림 + " bp" | "+299 bp" |
| HHI | `fmt.hhi(n)` | 정수 반올림, 콤마 없음 | "636" |
| 증감율 | `fmt.pctChange(n)` | 부호 강제 + 소수점 2자리 + "%" | "+3.45%" |
| 집중도 | `fmt.topN(n, share)` | "상위 N종 XX.X%" | "상위 10종 42.6%" |
| 날짜 | `fmt.date(s)` | YYYY-MM-DD 고정 | "2026-04-14" |
| null | `fmt.na()` | "—" (em dash) | "—" |

### 단위 정책
- AUM은 억원 고정. 조원 자동 전환 금지.
- `preferUnit` 옵션은 열어두되 기본값 "억원" 불변.
- 1조 넘는 상품이 나올 경우 재논의.

### 적용 범위
- 모든 차트 레이블·툴팁·KPI 카드·표·본문 숫자
- 현재 배포본(report.pdf, index.html)은 인라인 방식으로 이미 동일 규칙 준수
- 차기 재빌드부터 공용 `fmt.*` 함수로 통합 리팩토링

### data-analyst 가이드
- 수치 산출 시 소수점 자리수만 spec에 기입
- 단위 문자열(억원, %, %p, bp)은 렌더러가 부여
